...
#### Publish your solution in a public repository and send us a link.
...
The login endpoint is handled by the controller `app/Http/Controllers/Api/SecurityController.php`.
...
There is a controller with the `titles()` method in the `app/Http/Controllers/Api/MovieController.php`.
...
#### Make the route `/api/movies/titles` available only to authenticated users. Use the token generated in Task 1 for authentication.